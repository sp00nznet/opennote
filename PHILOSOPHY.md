...
1. **The problem is solved and the solution is published.** RFB is a protocol
   ...
   its own website. ECMA-376 is the `.docx` format, published and free to
   download; [MS-DOC] is the old one. SCSI MMC, VSS, VHDX,
   `AttachVirtualDisk`, the ATA and NVMe sanitize commands — documented,
   stable, and in most cases already sitting inside Windows, paid for, waiting
   to be called.
...
3. **The payment prompt is placed where it hurts most.** The tool reads your
   ...
   out. The PDF site fills in the form you were sent and watermarks the
   download. The price is not attached to the work. It is attached to your
   worst day.
...
There is a fourth shape, and opennote is the answer to it: **the thing used to
be in the box, and then it was taken out.** WordPad shipped with Windows for
thirty years and was deleted in Windows 11 24H2. Nothing replaced it. The
suggested replacement is a subscription, and the file formats it opened —
`.rtf`, `.docx` — did not go anywhere. That is not a market appearing. That is
a market being made.
...
| [opennote](https://github.com/sp00nznet/opennote) | The word processor Windows removed. Reads and writes `.rtf` and `.docx`, lays them out on real pages, prints and exports PDF — and opens, fills in and signs a PDF. |
